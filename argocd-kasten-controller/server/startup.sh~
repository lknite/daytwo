...
if [ ! -f /usr/local/bin/k10multicluster ]; then
  ...
  pushd /tmp > /dev/null
  curl -LO https://github.com/kastenhq/external-tools/releases/download/$INSTALL_VERSION/k10multicluster_$INSTALL_VERSION_linux_amd64.tar.gz --insecure && \
    tar zxvf k10multicluster_$INSTALL_VERSION_linux_amd64.tar.gz && \
