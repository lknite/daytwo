...
{
    public class VClusterK8sController
    {
        static string api = "vcluster";
        static string group = "infrastructure.cluster.x-k8s.io";
        static string version = "v1alpha1";
        ...
        public async Task ProcessAdded(CrdVCluster tkc)
        {
            ProcessModified(tkc);
        }
        public async Task ProcessModified(CrdVCluster tkc)
        {
            ...
            foreach (var ns in namespaces)
            {
                ...
                try
                {
                    ...
                    CustomResourceList<CrdVCluster> t =
                        await generic.ListNamespacedAsync<CustomResourceList<CrdVCluster>>(ns.Name());
        ...
        public async Task ProcessDeleted(CrdVCluster tkc)
        {
        ...
        public static async Task<KubernetesClientConfiguration> GetClusterKubeConfig(string clusterName, string clusterNamespace)
        {
            ...
            int asdf = await Globals.service.kubeclient.NamespacedPodExecAsync("argocd-server", "argocd", "argocd-server", cmds, true, One, Globals.cancellationToken);
