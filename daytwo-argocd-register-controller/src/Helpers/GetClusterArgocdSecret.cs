...
namespace daytwo.Helpers
{
    public partial class Main
    {
        ...
        {
            //Globals.log.LogInformation(new EventId(Thread.CurrentThread.ManagedThreadId), "- GetClusterSecret, clusterName: "+ clusterName);
            V1SecretList secrets = Globals.service.kubeclient.ListNamespacedSecret(Globals.service.argocdNamespace);

            //Globals.log.LogInformation(new EventId(Thread.CurrentThread.ManagedThreadId), "- argocd cluster secrets:");
            foreach (V1Secret secret in secrets)
            {
                ...
                //Globals.log.LogInformation(new EventId(Thread.CurrentThread.ManagedThreadId), "  - name: " + name +", tkcName: "+ tkc.Metadata.Name);
                if (name != clusterName)
                {
                    continue;
                }
